...
async def scrape() -> tuple[list[Match], list[Match]]:
    async with async_playwright() as pw:
        ...
        try:
            ...
            # Keep only the three competitions that belong to this dashboard.
            # Flashscore's results page also exposes friendlies, President Cup,
            # and other competitions; those must never inflate the season totals.
            all_matches = [
                m for m in normalized.values()
                if m.date
                and SEASON_START <= date.fromisoformat(m.date) <= SEASON_END
                and m.competition_short in TARGET_COMPETITIONS
            ]

            # Merge with the known-good season baseline. This prevents a partial
            # Flashscore page (e.g. only the next 16 league fixtures) from
            # shrinking the dashboard's season schedule. Scraped records win
            # when the same date/home/away/competition is present.
            existing = list(read_existing(FIXTURES_FILE, "fixtures").values()) + list(read_existing(RESULTS_FILE, "results").values())
            baseline = [
                m for m in existing
                if m.date and SEASON_START <= date.fromisoformat(m.date) <= SEASON_END
                and m.competition_short in TARGET_COMPETITIONS
            ]
            by_key = {}
            for m in baseline:
                by_key[(m.date, normalize_team_text(m.home).lower(), normalize_team_text(m.away).lower(), m.competition_short)] = m
            for m in all_matches:
                key=(m.date, normalize_team_text(m.home).lower(), normalize_team_text(m.away).lower(), m.competition_short)
                old=by_key.get(key)
